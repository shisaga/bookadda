<?php
    include '../config.php';
    include 'header_rent.php';
    $id = $_GET['id'];
    $_SESSION['book_id'] = $id;
    $output ='';
    $desc='';
    $sql = "SELECT * FROM rent_books WHERE id= $id";
    $result = mysqli_query($link,$sql);

					while ($row = mysqli_fetch_array($result)) {
						
						$output .='<div  id="product">
							<a href="book_details.php?id='.$row['id'].'"><img src="'.$row['img'].'" width="80%" height="200"></a>
							<h5 style="font-size:medium">'.$row['book_name'].'</h5>
							<h5 style="font-size:x-small">'.$row['author'].'</h5>
							<h5 style="font-size:larger;color:red">&#x20b9 '.$row['price'].'/Day</h5>
							...
							<button type="button" name="add_to_cart" id="'.$row['id'].'" class="btn btn-primary" style="width:80%;background-color:crimson">ADD TO CART </button>';
							$output.='</form> </div>';
...
<div class="row">
    ...
    <div class="col-sm-12 col-lg-9 col-9 ">
        <h5 style="font-weigth:bold">Description</h5>
        <hr>
        <?php echo"$desc" ?>
        <br><br>
                    <hr>
